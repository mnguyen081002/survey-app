...
//     const npsScore =
//       satisfactionScore > 3 ? randomIntFromInterval(7, 10) : randomIntFromInterval(1, 6);
//     data.push({
...
export default function TableViewComponent({ surveyJson, surveyResults }: TableViewComponentProps) {
  ...
  if (!surveyDataTable && !!survey) {
    const surveyDataTable = new Tabulator(survey, surveyResults, { jspdf: jsPDF, xlsx: XLSX });
  ...
  useEffect(() => {
    ...
    return () => {
      const element = document.getElementById('surveyDataTable');
      if (element) {
        element.innerHTML = '';
      }
    };
